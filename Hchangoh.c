...
if ((f1 = open(srcname, O_RDONLY, 0)) == -1){ //open the source folder f1 for reading only. R_RDONLY is read only
	printf("Can't open %s \n", srcname);// error messeage will be printed if system call fails and returns -1
	return 4;
}

if ((f2 = creat(tgtname, 0644)) == -1){ // create the target folder f2. 0644 is (owning)user : read and write, Group : read, Other: read
...
//cpfile.exe, srcname, tgt1file, tgt2file
	if (argc != 4){ // number of argument should be 4 and file from the chil process should be placed 4th argument.
...
if (pid<0){ // if pid is less than 0 it fails to create the child process
	fprintf(stderr, "Fork Failed"); // when it fails, error messege prints
	return 1;
	
}
else if (pid ==0){ //  child is processed. child process has its own pid.
